...
| **8** | **Foreground-to-Background WorkManager Handoff** | Reliability | Auto-handoff long manual syncs (>1 year range) to an expedited `OneTimeWorkRequest` if the user backgrounds or leaves the app during a sync. | 📋 **Planned** |
| **9** | **Automated Calendar Backup (.ics / JSON)** | Data Protection | Scheduled local backups of calendar snapshots to standard `.ics` (iCalendar) or encrypted `.json` files. | 📋 **Planned** |
